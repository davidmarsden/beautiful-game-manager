...
test('Open Market listings do not call the heavyweight transfer-deals read', async () => {
  ...
  assert.match(endpoint, /get_manager_transfer_market_for_user/);
  assert.doesNotMatch(endpoint, /settleDueTransfers|get_manager_transfer_lifecycle_for_user|get_manager_transfer_exchange_legs_for_user|get_manager_transfer_agreed_changes_for_user/);
});
