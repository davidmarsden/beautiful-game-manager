...
test('under-19 candidates below the readiness threshold cannot consume viability signings', () => {
  const result = planCanonicalRegistrationRepair(world(), {
    freeAgentCandidates: [
      { source_index: 0, player: { tbg_player_id: 'prospect-gk', display_name: 'Prospect Keeper', position: 'GK', age: 18, underlying_ability_rating: 79, club_id: null, contract_id: null } },
  ...
  assert.equal(result.preview.external_free_agents_materialised, 1);
  assert.equal(result.world.squad_cycle.players['prospect-gk'], undefined);
