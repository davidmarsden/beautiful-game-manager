...
function profileIdentity(player, club, pinkFinalLink) {
  ...
  const availability = availabilityState(player);
  return `<div class="tbg-player-card"><div class="tbg-player-rating" aria-label="TBG rating ${escapeHtml(rating(player))}"><span>TBG</span><strong>${escapeHtml(rating(player))}</strong></div><div class="tbg-player-summary"><span class="status-label">TBG PLAYER PROFILE</span><h2>${escapeHtml(playerName(player))}</h2><p class="tbg-player-role">${escapeHtml(position(player))}</p><p class="tbg-player-club"><span>Current TBG club</span><strong>${escapeHtml(currentClub(player, club))}</strong></p><div class="tbg-player-chips"><span>Age ${escapeHtml(player.age ?? '—')}</span>${nation ? `<span>${escapeHtml(nation)}</span>` : ''}<span class="status-${availability.tone}">${escapeHtml(availability.label)}</span></div></div><div class="tbg-player-actions">${pinkFinalLink ? `<a class="tbg-pink-final-link" href="${escapeHtml(pinkFinalLink)}" target="_blank" rel="noopener"><span>Real-world profile</span><strong>View in The Pink Final ↗</strong></a>` : '<div class="tbg-pink-final-link unavailable"><span>Real-world profile</span><strong>Not published yet</strong></div>'}</div></div>`;
}
