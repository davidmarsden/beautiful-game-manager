...
function ensureMatchCentre() {
  ...
  modal.id = 'matchCentreModal';
  modal.className = 'match-centre-modal';
  modal.hidden = true;
  modal.innerHTML = `<div class="match-centre-shell" role="dialog" aria-modal="true" aria-labelledby="matchCentreTitle"><header><div><span class="teletext-kicker">THE BEAUTIFUL GAME // MATCH CENTRE</span><h2 id="matchCentreTitle">MATCH REPORT</h2></div><button id="closeMatchCentre" type="button" aria-label="Close match report">×</button></header><div id="matchCentreContent" class="match-centre-content"></div></div>`;
...
function renderMatchCentre(data) {
  ...
  content.innerHTML = `
    <section class="teletext-scoreboard">
      <div><span>HOME</span><strong>${mcEscape(fixture.home_club_name)}</strong></div>
      <div class="teletext-score"><span>FT</span><b>${mcEscape(fixture.home_score)}-${mcEscape(fixture.away_score)}</b></div>
      <div><span>AWAY</span><strong>${mcEscape(fixture.away_club_name)}</strong></div>
    </section>
    ...
    <section id="matchTabReport" class="match-tab active">
      <div class="teletext-grid">
        <article class="teletext-panel"><h3>EVENTS</h3><ol class="event-list">${events.length ? events.map((event) => `<li><time>${mcEscape(event.minute)}'</time><span class="event-side ${mcEscape(event.side)}">${mcEscape(event.side)}</span><strong>${mcEscape(eventText(event))}</strong></li>`).join('') : '<li>No recorded match events.</li>'}</ol></article>
        <article class="teletext-panel"><h3>MATCH STATISTICS</h3><div class="stat-line"><span>${stats.home?.possession ?? '—'}%</span><b>POSSESSION</b><span>${stats.away?.possession ?? '—'}%</span></div><div class="stat-line"><span>${stats.home?.shots ?? '—'}</span><b>SHOTS</b><span>${stats.away?.shots ?? '—'}</span></div><div class="stat-line"><span>${stats.home?.shots_on_target ?? '—'}</span><b>ON TARGET</b><span>${stats.away?.shots_on_target ?? '—'}</span></div><div class="match-meta">Matchday ${mcEscape(fixture.matchday ?? '—')}<br>${new Intl.DateTimeFormat('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }).format(new Date(fixture.played_at))}</div></article>
      </div>
    </section>
    <section id="matchTabReplay" class="match-tab">
      <div class="replay-console"><div class="replay-clock" id="replayClock">00'</div><div class="replay-score"><span>${mcEscape(fixture.home_club_name)}</span><b id="replayScore">0-0</b><span>${mcEscape(fixture.away_club_name)}</span></div><div id="replayFeed" class="replay-feed"><p>Press START to replay the saved match.</p></div><div class="replay-controls"><button id="replayStart" type="button">START</button><button id="replayPause" type="button">PAUSE</button><button id="replaySkip" type="button">FULL TIME</button><label>Speed<select id="replaySpeed"><option value="900">1×</option><option value="450">2×</option><option value="180">5×</option></select></label></div></div>
    </section>
  ...
  modal.hidden = false;
  content.querySelectorAll('[data-match-tab]').forEach((button) => button.addEventListener('click', () => {
    content.querySelectorAll('[data-match-tab]').forEach((item) => item.classList.toggle('active', item === button));
    content.querySelectorAll('.match-tab').forEach((section) => section.classList.toggle('active', section.id === `matchTab${button.dataset.matchTab[0].toUpperCase()}${button.dataset.matchTab.slice(1)}`));
  }));
  setupReplay(data);
}
...
function setupReplay(data) {
  ...
  replayState = { minute: 0, home: 0, away: 0, events, nextEvent: 0, data };
  const tick = () => {
    ...
    while (replayState.nextEvent < events.length && Number(events[replayState.nextEvent].minute) <= replayState.minute) {
      ...
      feed.insertAdjacentHTML('afterbegin', `<p class="replay-event ${mcEscape(event.event_type)}"><time>${mcEscape(event.minute)}'</time> ${mcEscape(eventText(event))}</p>`);
    }
    document.getElementById('replayClock').textContent = `${String(Math.min(replayState.minute, 90)).padStart(2, '0')}'`;
    document.getElementById('replayScore').textContent = `${replayState.home}-${replayState.away}`;
    if (replayState.minute >= 90) { clearInterval(replayTimer); replayTimer = null; document.getElementById('replayFeed').insertAdjacentHTML('afterbegin', '<p class="full-time">90\' FULL TIME</p>'); }
  };
  document.getElementById('replayStart').addEventListener('click', () => {
    if (replayTimer) return;
    if (replayState.minute >= 90) { replayState.minute = 0; replayState.home = 0; replayState.away = 0; replayState.nextEvent = 0; document.getElementById('replayFeed').innerHTML = ''; }
    replayTimer = setInterval(tick, Number(document.getElementById('replaySpeed').value));
  });
  document.getElementById('replayPause').addEventListener('click', () => { clearInterval(replayTimer); replayTimer = null; });
  document.getElementById('replaySkip').addEventListener('click', () => { clearInterval(replayTimer); replayTimer = null; while (replayState.minute < 90) tick(); });
...
document.addEventListener('click', (event) => {
  const target = event.target.closest('[data-match-centre]');
  if (!target) return;
  event.preventDefault();
  openMatchCentre(target.dataset.matchCentre);
});
document.addEventListener('keydown', (event) => {
  if (event.key === 'Escape') closeMatchCentre();
  const target = event.target.closest?.('[data-match-centre]');
  if (target && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); openMatchCentre(target.dataset.matchCentre); }
});
