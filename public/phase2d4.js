...
function renderMatchCentre(data) {
  ...
  <section id="matchTabReport" class="match-tab active"><div class="teletext-grid"><article class="teletext-panel"><h3>EVENTS</h3><ol class="event-list">${events.length ? events.map((event) => `<li><time>${mcEscape(event.minute)}'</time><span class="event-side ${mcEscape(event.side)}">${mcEscape(event.side === 'neutral' ? '' : event.side)}</span><strong>${mcEscape(eventText(event))}</strong></li>`).join('') : '<li>No recorded match events.</li>'}</ol></article><article class="teletext-panel"><h3>MATCH STATISTICS</h3><div class="stat-line"><span>${stats.home?.possession ?? '—'}%</span><b>POSSESSION</b><span>${stats.away?.possession ?? '—'}%</span></div><div class="stat-line"><span>${stats.home?.shots ?? '—'}</span><b>SHOTS</b><span>${stats.away?.shots ?? '—'}</span></div><div class="stat-line"><span>${stats.home?.shots_on_target ?? '—'}</span><b>ON TARGET</b><span>${stats.away?.shots_on_target ?? '—'}</span></div><div class="match-meta">Matchday ${mcEscape(fixture.matchday ?? '—')}<br>${new Intl.DateTimeFormat('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }).format(new Date(fixture.played_at))}</div></article></div></section>
