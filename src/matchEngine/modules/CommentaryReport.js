...
export const COMMENTARY_REPORT_VERSION = 'tbg-commentary-report-v0.1';
...
function clubName(team, fallback) {
  return text(team?.club_name || team?.name || team?.display_name || team?.club_id) || fallback;
}

function playerLookup(quality = {}) {
  const lookup = new Map();
  for (const side of ['home', 'away']) {
    for (const player of quality?.[side]?.starters || []) {
      lookup.set(String(player.player_id), player.display_name || player.player_id);
    }
  }
  return lookup;
}
...
function eventSentence(event, names, clubs) {
  ...
  switch (event.type) {
    ...
    case 'penalty': return `${club} are awarded a penalty.`;
...
function summary(resolution, clubs) {
  const home = resolution.statistics.home;
  const away = resolution.statistics.away;
  ...
  if (dominant) parts.push(`${dominant} produced the greater shot volume.`);
  else parts.push('The shot count was level.');
...
function talkingPoints(resolution, tactical = {}, quality = {}) {
  ...
  const stats = resolution.statistics;
  const tacticalEdge = number(tactical?.matchup?.net?.home_advantage, 0);
  if (Math.abs(tacticalEdge) >= 0.03) {
    points.push(tacticalEdge > 0 ? 'The home tactical plan held a measurable matchup edge.' : 'The away tactical plan held a measurable matchup edge.');
  }
  ...
  if (stats.home.shots_on_target !== stats.away.shots_on_target) {
    points.push(stats.home.shots_on_target > stats.away.shots_on_target ? 'The home side put more efforts on target.' : 'The away side put more efforts on target.');
  }
  if (score.home === score.away) points.push('Neither side converted enough of its opportunities to separate the teams.');
    ...
    if (stats[winner].expected_goals < stats[loser].expected_goals) points.push('The winner prevailed despite trailing on expected goals.');
  }
  return points.slice(0, 4);
}

function keyEvents(events) {
  const priority = { goal: 6, red_card: 5, penalty: 4, injury: 4, big_chance: 3, yellow_card: 2, set_piece: 1, shot: 0 };
  return [...events]
    .sort((left, right) => (priority[right.type] || 0) - (priority[left.type] || 0) || left.minute - right.minute)
    .slice(0, 12)
...
export function resolveCommentaryReport(contract, resolution, tactical = {}, quality = {}, fatigue = {}) {
  ...
  const clubs = {
    home: clubName(contract?.teams?.home, 'Home'),
    away: clubName(contract?.teams?.away, 'Away')
  };
  const names = playerLookup(quality);
  const selected = keyEvents(resolution.official_event_stream || []);
  const commentary = selected.map((event) => deepFreeze({
    minute: event.minute,
    side: event.side,
    type: event.type,
    event_id: event.event_id,
    text: eventSentence(event, names, clubs)
  }));

  return deepFreeze({
    version: COMMENTARY_REPORT_VERSION,
    fixture_id: contract?.fixture?.fixture_id || contract?.fixture?.id || null,
    headline: headline(resolution.score, clubs),
    summary: summary(resolution, clubs),
    score: resolution.score,
    clubs,
    commentary,
    talking_points: talkingPoints(resolution, tactical, quality),
    ...
    condition_context: {
      home_average_fitness: fatigue?.home?.team?.average_fitness ?? null,
      away_average_fitness: fatigue?.away?.team?.average_fitness ?? null
    },
    report_complete: true,
    public_contract_transition_pending: true,
    applied_to_public_result: false
  });
}

export function executeCommentaryReport(context) {
  const result = resolveCommentaryReport(
    context.contract,
    context.get('module_e_match_resolution'),
