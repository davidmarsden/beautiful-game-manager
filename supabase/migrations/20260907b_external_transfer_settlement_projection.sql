-- Route managed-world deals and external-market deals to different settlement
-- workers. External clubs are mirrored TPF identities, not simulated squad-cycle clubs.
...
create or replace function public.get_due_external_transfer_settlements(
  p_world_id text default null,
  p_limit integer default 10
) returns jsonb
language sql
...
grant execute on function public.get_due_transfer_settlements(text,integer)
  to service_role;
revoke all on function public.get_due_external_transfer_settlements(text,integer)
  from public, anon, authenticated;
grant execute on function public.get_due_external_transfer_settlements(text,integer)
  to service_role;
