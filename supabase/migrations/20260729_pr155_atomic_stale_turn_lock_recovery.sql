-- PR #155: recover abandoned canonical turn locks atomically.
-- This RPC is service-role only because it mutates canonical scheduler state.
...
begin
  ...
  -- The canonical row is locked above. No competing recovery can mutate dependent
  ...
  -- A worker can die after claiming the world but before create_turn_run. Create
  -- authoritative failed-run lineage so the ordinary retry path never dead-ends.
