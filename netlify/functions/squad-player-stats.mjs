...
export default async (request) => {
  try {
    ...
    const worldId = String(payload.world_id || 'tbg-world-1').trim();
    const ids = [...new Set((Array.isArray(payload.player_ids) ? payload.player_ids : []).map((value) => String(value || '').trim()).filter(Boolean))];
    if (!ids.length) return json({ error: 'player_ids is required' }, 400);
    if (ids.length > 50) return json({ error: 'A maximum of 50 players can be requested at once' }, 400);

    const entries = await Promise.all(ids.map(async (playerId) => {
    ...
    return json({ world_id: worldId, players: Object.fromEntries(entries) });
  } catch (error) {
