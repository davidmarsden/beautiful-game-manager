...
export default async (request) => {
  try {
    ...
    return json({
      ...
      failed_at: failedRun?.completed_at || world.updated_at,
      failed_run_id: failedRun?.id || null,
      operation_id: operation?.operation_id || null,
      error: failedRun?.error_message || operation?.details?.scheduler_result?.error || 'The production turn failed without a recorded exception.',
      diagnostics: operation?.details?.scheduler_result?.diagnostics || null,
      ...
      recovery: failedRun
        ...
        : 'No matching failed turn record was found. Automatic retry is blocked; manual recovery is required.'
    });
  } catch (error) {
