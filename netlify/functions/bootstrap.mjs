...
export default async (request) => {
  try {
    ...
    const messages = await supabase(`/rest/v1/manager_messages?recipient_manager_id=eq.${encodeURIComponent(manager.id)}&select=id,message_type,subject,body,priority,created_at,read_at,related_fixture_id&order=created_at.desc&limit=25`, token).catch(() => []);
    const unread_count = messages.filter((m) => !m.read_at).length;

    if (!appointment) return json({ authenticated: true, user: { id: user.id, email: user.email }, manager, onboarding_required: !manager.profile_completed, appointment: null, no_assignment: true, messages, unread_count, navigation: ['Dashboard','Squad','Tactics','Schedule','Finances','Facilities','History','Transfers','Competitions','World'] });
