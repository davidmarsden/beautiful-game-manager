...
function squadProjection(player, index, ownership) {
  ...
  const seasonStartAge = number(player.season_start_age ?? player.age, null);
  const loanedOut = Boolean(loan.active ?? player.loaned_out);
  return {
    ...player,
    ...
    youth_eligible_at_season_start: seasonStartAge !== null && seasonStartAge <= 21,
    squad_registration: seasonStartAge !== null && seasonStartAge <= 21 ? "youth" : "first_team",
    loaned_out: loanedOut,
    loan_club_id: text(loan.club_id || player.loan_club_id) || null,
    loan_club_name: text(loan.club_name || player.loan_club_name) || null,
